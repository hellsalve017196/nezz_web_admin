...
<html lang="en">
              ...
              <div class="x_panel">
                <div class="row x_title">
                  ...
                  <div class="col-md-2" style="text-align: right;">                    
                    <select id="merchant" name="merchant" required="required" class="form-control col-md-2 col-xs-12">
                      <?php
                      $database = new Database;
                      $database->query("SELECT * FROM payment_methods;");
                      $merchants = $database->resultset();
                      foreach ($merchants as $merchant) :
                        echo "<option value='".$merchant["id"]."'>".strtoupper($merchant["payment_name"])."</option>";
                      endforeach;
                      ?>                      
                    </select>                    
                  </div>               
                ...
                <div class="row x_content">                  
                  <div id="echart_line-merchant-income" style="height:450px;"></div>
                </div>
  ...
  function loadChart() {
    ...
    if ($('#echart_line-merchant-income').length) {      
        
      var echartLine = echarts.init(document.getElementById('echart_line-merchant-income'), theme);

          $.ajax({
          url: "services/merchant_income.php",
          method: "POST",
          data: {
          ...
          success: function(data) {          
            ...
            if (response.status == "success") {              
              echartLine.setOption({
                title: {
                  text: 'Merchant: ' + $("#merchant option:selected").text() + ', Range: ' + $("#daterange").val(),
