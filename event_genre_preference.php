...
$pageTitle = "Event Genre Preference";
...
switch (strtoupper($get["a"])) {
  case "SAVE" :    
    if ($id == 0) {
      if ($post["name"] != "") {
        $database->query('SELECT * FROM genre WHERE Name = :Name;');
        $database->bind(':Name', $post["name"]);
        $uniqueName = $database->single();
        if ($uniqueName) Messages::set($pageTitle." already in use");
      }
      else Messages::set($pageTitle." is required");
    }    

    if (!Messages::hasError()) {
      if ($id == 0) {
        $database->query('INSERT INTO genre (Name) VALUES (:Name);');
        $database->bind(':Name', $post["name"]);
        $database->execute();        
      }
      else {
        $database->query('UPDATE genre SET Name = :Name WHERE Id = :Id;');
        $database->bind(':Name', $post["name"]);
        $database->bind(':Id', $id);
        $database->execute();
      }
  ...
  case "SAVEOK" :
    ...
    break;
  case "DELETE" :
    $database->query('DELETE FROM genre WHERE Id = :Id;');
    $database->bind(':Id', $id);
    $database->execute();
...
if ($id > 0) {
  $database->query('SELECT * FROM genre WHERE Id = :Id;');
  $database->bind(':Id', $id);
            ...
            <div class="col-md-12 col-sm-12 col-xs-12">
              <div class="x_panel">
                ...
                <div class="row x_content">
                  ...
                  <?php } else { ?>
                  <form id="eventgenreform" method="post" action="<?php echo $_SERVER['PHP_SELF'].'?a=save&id='.$id; ?>" class="form-horizontal form-label-left">
